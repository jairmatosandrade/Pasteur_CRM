...
@version 1.0
	...
	15 - MATA040 - SBM - Grupos de Produtos
	16 - MATA015 - SBE - Enderecos
/*/
User Function cargaTab()
	Local aArea := GetArea()
	//Dimensões da janela
	Local nJanAltu := 228
	Local nJanLarg := 716
	...
	Local oBtnObri
	Local oBtnArq
	Local 	cTitulo		:= "Carga Dados - Genérico"
	Private aIteTip := {}, aRotina := {}
	Private oSayArq, oGetArq, cGetArq := Space(99)
	Private oSayTip, oCmbTip, cCmbTip := ""
	Private oSayCar, oGetCar, cGetCar := ';'
	Private oSayReg, oGetreg, cGetReg := ';'
	Private oDlgPvt,oModel
	...
	cGetReg := "Campos Numéricos com separação de decimal, utilize o caracter '.'. Por exemplo: 5.20;"+STR_PULA
	cGetReg += "Campos do tipo Data, utilize ou o padrão YYYYMMDD ou o DD/MM/YYYY. Por exemplo: 20151025 ou 12/06/2024;"+STR_PULA
	cGetReg += " O caracter ';' (ponto e vírgula), nunca pode estar no fim da linha!"+STR_PULA
	cGetReg += STR_PULA
	cGetReg += "Rotinas que podem ser importadas:"+STR_PULA
	cGetReg += " 01-MATA070-SA6-Bancos"+Space(30)+"02-MATA030-SA1-Clientes"+STR_PULA
	cGetReg += " 03-MATA360-SE4-Condição de Pagamento"+Space(04)+"04-FINA040-SE1-Contas a Receber"+STR_PULA
	cGetReg += " 05-FINA050-SE2-Contas a Pagar"+Space(20)+"06-MATA020-SA2-Fornecedores"+STR_PULA
	cGetReg += " 07-FINA010-SED-Naturezas"+Space(28)+"08-MATA010-SB1-Produtos"+STR_PULA
	cGetReg += " 09-MATA220-SB9-Saldo Inicial"+Space(24)+"10-MATA080-SF4-TES(Tipo de Entrada e Saída)"+STR_PULA
	cGetReg += " 11-MATA230-SF5-Tipo Movimentação "+Space(12)+"12-MATA050-SA4-Transportadoras"+STR_PULA
	cGetReg += " 13-OMSA060-DA3-Veículos"+Space(28)+"14-MATA040-SA3-Vendedores"+STR_PULA
	cGetReg += " 15-MATA035-SBM-Grupos de Produtos"+STR_PULA
	cGetReg += " 16-MATA015-SBM-Endereços"+STR_PULA

	//Criando a janela
	Define MSDialog oDlgPvt Title cTitulo From 008.2,010.3 To 039.0,101.5 Of GetWndDefault()
	//Grupo Parâmetros
	@ 003, 003 	GROUP oGrpPar TO 205, (nJanLarg/2) 	PROMPT "Parâmetros: " 		OF oDlgPvt COLOR 0, 16777215 PIXEL
	//Caminho do arquivo
	...
	//Tipo de Importação
	@ 028, 006 SAY        oSayTip PROMPT "Rotina:"          SIZE 060, 007 OF oDlgPvt PIXEL
	...
	@ 060, 006 SAY        oSayReg PROMPT "Observações.:"            SIZE 060, 007 OF oDlgPvt PIXEL
	@ 060, 070 SAY        oSayReg PROMPT  cGetReg                   SIZE 250, 150 OF oDlgPvt PIXEL

	//Grupo Ações
	@ 205, 003 GROUP  oGrpAco To nJanAltu,(nJanLarg/2) Of oDlgPvt LABEL "Ações:" PIXEL

	//Botões
	@ 211, (nJanLarg/2)-(63*1)  BUTTON oBtnSair PROMPT "Sair"          SIZE 60, 014 OF oDlgPvt ACTION (oDlgPvt:End()) PIXEL
	@ 211, (nJanLarg/2)-(63*2)  BUTTON oBtnImp  PROMPT "Importar"      SIZE 60, 014 OF oDlgPvt ACTION (Processa({|| fConfirm(1) }, "Aguarde...")) PIXEL
	@ 211, (nJanLarg/2)-(63*3)  BUTTON oBtnObri PROMPT "Camp.Obrig."   SIZE 60, 014 OF oDlgPvt ACTION (Processa({|| fConfirm(2) }, "Aguarde...")) PIXEL
...
Static Function fConfirm(nTipo)
	...
	ElseIf cCmbTip == "15"
		...
		//Grupos de Produtos
	ElseIf cCmbTip == "16"		
		cRotina	:= "FwMvcRotAuto( oModel, 'SBE', 3, { { 'MdFieldSBE', aDados } } )" 
		cTabela	:= "SBE"
		cCampoChv	:= "BE_LOCAL"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 4
